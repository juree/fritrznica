...
def ucilnica_login(username, password):
    ...
    try:
        resp = requests.post(UCILNICA_LOGIN_URL,
        ...
        if resp.url == UCILNICA_EDIT_URL:
            match = re.search(
                '<a href="https://ucilnica.fri.uni-lj.si'
                '/user/profile.php\?id=(\d+)">(.*?)</a>',
                resp.content)  # id, ime priimek
            anothermatch = re.search('value="(.*?)" id="id_idnumber"',
                                     resp.content)
